...
module.exports.fastaDataOne= catchAsync(async function(req, res){
   ...
   // fasta file one code start
    ...
    const fastaDataOne = await fs.readFile('./data/Trinity_C_NormalCells.Trinity1.fasta', 'utf-8')
    const seqOne = fasta.parse(fastaDataOne) // this line will convert fasta data to javascript object
  

  
   if( seqOne ){
     return res.status(200).json({   // json method will convert out javascript object to json object
       message:"Success",
       status:'ok',
       seqOne:seqOne,
...
module.exports.fastaDataTwo=catchAsync(async function(req, res){
   ...
   const fastaDataTwo = await fs.readFile('./data/Trinity_C_NormalCells.Trinity2.fasta', 'utf-8')
